...
<div class="columns">
  <div class="column is-3 has-background-primary has-text-centered">
    ...
    <figure class="image is-square m-5">
        <img @click="cookies++" class="is rounded" src="https://purepng.com/public/uploads/large/purepng.com-cookiecookieamericandelicioussnacksweetyummybiscuit-215258865264pxyk.png">
    </figure>
  </div>
  <div class="column is-7 has-background-info">Second column</div>
  <div class="column is-2 has-background-warning">
    <button class="button is-primary is-large is-fullwidth" v-for="building in buildings" :disabled="cookies < building.price" @click="buyBuilding(building)">{{ building.name }} 🍪 {{ building.price }}</button>
  </div>
</div>
</template>